# -*- coding: utf-8 -*-
"""rsitmd_svm_feature_pytorch.ipynb (augmentation clone)

Automatically generated by Colab.

This version adds stronger train-time augmentation before feature extraction so you can compare the classical SVM pipeline against the non-augmentation version.
...
print(f"Train Accuracy: {accuracy_score(y_train, y_train_pred):.4f} - BalAcc: {balanced_accuracy_score(y_train, y_train_pred):.4f} - Macro F1: {f1_score(y_train, y_train_pred, average='macro', zero_division=0):.4f}")
print(f"Val Accuracy  : {accuracy_score(y_val, y_val_pred):.4f}   - BalAcc: {balanced_accuracy_score(y_val, y_val_pred):.4f}   - Macro F1: {f1_score(y_val, y_val_pred, average='macro', zero_division=0):.4f}")

if X_test is not None:
    y_test_pred = svm.predict(X_test)
    print(f"Test Accuracy : {accuracy_score(y_test, y_test_pred):.4f}   - BalAcc: {balanced_accuracy_score(y_test, y_test_pred):.4f}   - Macro F1: {f1_score(y_test, y_test_pred, average='macro', zero_division=0):.4f}")
    print("\nTest Classification Report:\n", classification_report(y_test, y_test_pred, labels=range(len(class_names)), target_names=class_names, zero_division=0))
    
    cm = confusion_matrix(y_test, y_test_pred, labels=range(len(class_names)))
    plt.figure(figsize=(10, 8))
    sns.heatmap(cm, annot=True, fmt='d', cmap='Greens', xticklabels=class_names, yticklabels=class_names)
    plt.title('End-to-End SVM Test Confusion Matrix')
    plt.xlabel('SVM Predicted'); plt.ylabel('Actual')
    plt.xticks(rotation=45, ha='right'); plt.tight_layout(); plt.savefig('svm_confusion_matrix.png'); plt.show()
    
# 6.5 LEARNING CURVES
print("\nGenerating Learning Curves (over dataset size)...")
train_sizes, train_scores, val_scores = learning_curve(
    svm, X_train, y_train, cv=3, scoring='f1_macro', n_jobs=-1,
...
plt.plot(train_sizes, train_mean, 'o-', color="r", label="Training Macro F1")
plt.plot(train_sizes, val_mean, 'o-', color="g", label="Cross-validation Macro F1")
plt.fill_between(train_sizes, train_mean - train_std, train_mean + train_std, alpha=0.1, color="r")
plt.fill_between(train_sizes, val_mean - val_std, val_mean + val_std, alpha=0.1, color="g")
plt.title("SVM Learning Curves (Data Size Validation)")
...
# 7.5 SAVE SVM REPORT
if X_test is not None:
    cls_report = classification_report(y_test, y_test_pred, labels=range(len(class_names)), target_names=class_names, zero_division=0, output_dict=True)
    report_path = "svm_feature_report.json"
    ...
    print(f"✅ Saved SVM evaluation report to {report_path}")
