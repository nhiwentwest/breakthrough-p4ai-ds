...
with st.expander("🎛️ Visual controls", expanded=False):
    ...
    with c1:
        chart_w = st.slider("Base width", 2.4, 12.0, 8.0, 0.2, key="text_chart_w")
    with c2:
        chart_h = st.slider("Base height", 1.6, 8.0, 4.5, 0.2, key="text_chart_h")
    with c3:
        chart_scale = st.slider("Global scale", 0.45, 2.0, 1.2, 0.02, key="text_chart_scale")
    with c4:
        chart_panel = st.slider("Panel width", 0.45, 1.0, 0.85, 0.05, key="text_chart_panel")
    with c5:
        font_scale = st.slider("Font scale", 0.45, 2.0, 1.3, 0.05, key="text_font_scale")
